<?php
require_once(SITE_PATH.'/apps/taobaoke/Lib/Model/TaobaokeModel.class.php');
class TaobaokeOperateModel extends TaobaokeModel{
    ...
    function deleteMini($id,$uid) {
        if( $info = $this->where("weibo_id=$id AND uid=$uid")->find() ){
            if($info['isdel'] == 0 && $this->setField('isdel',1,'weibo_id='.$info['weibo_id'].' AND isdel=0')){
                ...
                //同时删除喜欢
                D('TaobaokeFav')->where('weibo_id='.$info['weibo_id'])->delete();
                //同时删除评论
                D('TaobaokeComment')->setField('isdel',1,'weibo_id='.$info['weibo_id']);
            }elseif($info['isdel'] == 1 && $this->where('weibo_id='.$info['weibo_id'].' AND isdel=1')->delete()){
                //同时彻底删除评论
                D('TaobaokeComment')->where('weibo_id='.$info['weibo_id'])->delete();
            }else{
                return false;
            }
            return true;
    ...
    public function getSpaceList($uid, $type, $bc_id) {
        ...
        // 去除被举报的微博
        if (($denounce_ids = model('Denounce')->getIdsDenounce('taobaoke','str')))
            $map.=" AND weibo_id NOT IN ( {$denounce_ids} )";
