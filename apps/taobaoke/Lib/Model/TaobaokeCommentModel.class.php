<?php
class TaobaokeCommentModel extends Model {
    var $tableName = 'taobaoke_comment';
    //发布评论
    function addcomment($data){
        if( $id= $this->add( $data ) ){
            D('Taobaoke')->setInc('comment', 'weibo_id='.$data['weibo_id'] );
            return $id;
        }else{
            return false;
        }
    ...
    public function doaddcomment($uid, $post, $api=false)
    {
        ...
        $data['ctime']   = time();
        $miniInfo = D('Taobaoke')->where('weibo_id='.$data['weibo_id'].' AND isdel=0')->find();
        ...
        if ( $comment_id = $this->addcomment( $data ) ){
            ...
            if( $post['transpond'] != 0 ){
                if($miniInfo['transpond_id']!=0){
                    ...
                    $transpondInfo = M('taobaoke')->where('weibo_id='.$miniInfo['transpond_id'].' AND isdel=0')->find();
