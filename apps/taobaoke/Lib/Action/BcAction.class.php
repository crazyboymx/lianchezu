...
class BcAction extends Action{
    ...
    public function setBcTab(){
        if(is_numeric($_REQUEST['gid'])){
            $title = D('TaobaokeBc')->getField('title',"bc_id={$_REQUEST['gid']}");
            $acid = D('TaobaokeBc')->getField('ac_id',"bc_id={$_REQUEST['gid']}");
        ...
        $acdisplay = D('TaobaokeAc')->order('`display_order` ASC')->findAll();
    ...
    public function setBc(){
        ...
        $acid = $_REQUEST['acid'];
        if(!$_REQUEST['gid']){
            $res = D('TaobaokeBc')->setBc($uid,$title,$acid);
        }else{
            $gid = $_REQUEST['gid'];
            // $acid = $_REQUEST['acid'];
            $res = D('TaobaokeBc')->setBc($uid,$title,$acid,$gid);
        }
        echo $res;
    }
    ...
    public function deleteBc($uid,$gid){
        $gid = $_REQUEST['gid'];
        $res = D('TaobaokeBc')->deleteBc($this->mid,$gid);
