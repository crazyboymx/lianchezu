...
class OperateAction extends Action{
    ...
    function publish(){
        ...
        if( $id ){
            ...
            X('Credit')->setUserCredit($this->mid,'add_taobaoke');
    ...
    function transpond() {
        ...
        $map['uid']=$this->mid;
        $bcdisplay = M('taobaoke_bc')->where($map)->order('bc_id DESC')->findAll();
        $this->assign('bcdisplay',$bcdisplay);
        $nobc= M('taobaoke_bc')->getField('bc_id',$map);
        ...
        if($_POST){
            ...
            if( $id = $pWeibo->transpond($this->mid,$post) ){
                $data = $pWeibo->getOneLocation($id);
                X('Credit')->setUserCredit($this->mid,'forward_taobaoke')
                    ->setUserCredit($data['expend']['uid'],'forwarded_taobaoke');
                $this->assign('data',$data);
            ...
            if( $info['transpond_id'] ){
                $info['transponInfo'] = D('Taobaoke')->field('weibo_id,uid,content,type_data')->where('weibo_id='.$info['transpond_id'])->find();
            }else{
