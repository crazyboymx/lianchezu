...
//---------------------仿知美二次开发 
//微博翻页点击监控
function welcome_index_weibo_page(){
	$("#page_nav a").click(function(o){
		var $a = $(o.target);
		var url = $a.attr("href");
		welcome_index_weibo(url);
		return false;
	});
}
//微博列表读取
function welcome_index_weibo(url){
	var url = url?url:U('home/welcome/index_weibo');
	var $square_list = $('#square_list');
	$square_list.html('<img src="'+_THEME_+'/images/loader.gif" style="margin:80px 50%;" />');
	//setTimeout(function(){if($square_list.html()=='')$square_list.html('<span style="margin:100px 42%;">获取微博失败！</span>');},10000);
	$.post(url,{},function(html){
		if(html == -1){
			$square_list.html('<span style="margin:100px 50%;">微博列表为空！</span>');
