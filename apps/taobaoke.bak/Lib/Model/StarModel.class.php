...
class StarModel extends Model {
	protected $tableName = 'taobaoke_weibo_star';
	protected $group_list;

	//获取某一父级分组下的子级分组列表   PS：顶级分组的父级为0
	public function getGroupList($tid=0){
		$map['top_group_id'] = intval($tid);
		$list = M('taobaoke_weibo_star_group')->where($map)->order('display_order ASC,star_group_id DESC')->findAll();
	...
	public function getStarGroup(&$star) {
		...
		$_star_group = M('taobaoke_weibo_star_group')->find(intval($star['star_group_id']));
		$star['star_group_title'] = $_star_group['title'];
		
		if($_star_group['top_group_id']!=0){		
			$star['top_group_id']    = $_star_group['top_group_id'];
			$star['top_group_title'] = M('taobaoke_weibo_star_group')->getField('title',"star_group_id={$_star_group['top_group_id']}");
		}
	...
	public function addGroup($title,$tid=0){
		...
		$groupModel = M('taobaoke_weibo_star_group');
	...
	public function addStar($uid,$gid){
		...
		//检测分组是否存在
		$gid = M('taobaoke_weibo_star_group')->getField('star_group_id',"star_group_id={$gid}");
		if($gid){
