...
class FollowModel extends Model{
	var $tableName = 'taobaoke_weibo_follow';
	...
	public function unfollow($mid, $fid, $type = 0)
	{
		...
		if ($this->where($map)->delete()) { // 取消成功
			if ($map['type'] == 0) { // 取消关注用户时
				...
				M('taobaoke_weibo_follow_group_link')->where("follow_id={$follow_id} AND uid={$map['uid']}")->delete();
	...
	function getList( $uid , $operate ,$type=0 ,$gid=NULL){
		...
		foreach ($list['data'] as $k=>$v){
			$list['data'][$k]['mini'] = M('taobaoke_weibo')->where('uid='.$v['fid'].' AND type='.$type)->order('weibo_id DESC')->find();
    	...
    	if ($key) {
    		...
    		D('User', 'home')->setUserObjectCache($list['data']);
    	   	...
    	   	foreach ($list['data'] as $k=>$v){
				...
				$list['data'][$k]['mini']        = M('taobaoke_weibo')->where('uid='.$v['uid'].' AND type=0 AND isdel = 0')->order('weibo_id DESC')->find();
