...
function getMiniNum($uid){
	return M('taobaoke_weibo')->where('uid='.$uid)->count();
}

//仿知美二次开发 获取微博条数
function getMiniNum_zhuan($uid){
	return M('taobaoke_weibo')->where('uid='.$uid.' AND isdel=0 AND type<>0')->count();
}

//获取关注数
function getUserFollow($uid){
	$count['following'] = M('taobaoke_weibo_follow')->where("uid=$uid AND type=0")->count();
	$count['follower']  = M('taobaoke_weibo_follow')->where("fid=$uid AND type=0")->count();
	return $count;
}
