...
class PublicAction extends Action{
    ...
    public function siteIndex()
    {
        ...
        // 热门晒费用
        $data['hot_showfee_list'] = D('Showfee', 'showfee')->getHotList();

        // 热门自驾游活动
        $data['hot_event_list'] = D('Event', 'event')->getHotList();

        // 热门购物
        $acList = D('TaobaokeAc', 'taobaoke')->getAllAc();
        $data['hot_taobaoke_list'] = array();
        foreach ($acList as $ac) {
            $data['hot_taobaoke_list'][$ac['ac_id']]['list'] = D('Taobaoke', 'taobaoke')->getHotList($ac['ac_id']);
            $data['hot_taobaoke_list'][$ac['ac_id']]['ac_title'] = $ac['title'];
            $data['hot_taobaoke_list'][$ac['ac_id']]['ac_id'] = $ac['ac_id'];
        }

        // 热门晒货
        $data['hot_poster_list'] = D('Poster', 'poster')->getHotList();
