 ...
 * 程序_小时代 提供
 */
class LootNewUserMedal extends BaseMedal {
	
	protected $_is_LootNewUser;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isLootNewUser($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isLootNewUser($uid) ) {
		...
		$result['next_level_time']	= $this->__isLootNewUser($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isLootNewUser($uid) {
		if (!isset($this->_is_LootNewUser)) {
			$this->_is_LootNewUser = false;
			 $map['uid']=$uid;	
  		 	 $counts=M('loot_count')->where($map)->getField('counts');
				if($counts > 10){
					$this->_is_LootNewUser = true;
				}
		}
		
		return $this->_is_LootNewUser;
	}
