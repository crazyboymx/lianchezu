 ...
 * 程序_小时代 提供
 */
class LootKingMedal extends BaseMedal {
	
	protected $_is_LootKing;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isLootKing($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isLootKing($uid) ) {
		...
		$result['next_level_time']	= $this->__isLootKing($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isLootKing($uid) {
		if (!isset($this->_is_LootKing)) {
			$this->_is_LootKing = false;
			 $map['uid']=$uid;	
  		 	 $counts=M('loot_count')->where($map)->getField('counts');
				if($counts > 100){
					$this->_is_LootKing = true;
				}
		}
		
		return $this->_is_LootKing;
	}
