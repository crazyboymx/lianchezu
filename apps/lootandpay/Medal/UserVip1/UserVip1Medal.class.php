 ...
 * 程序_小时代 提供
 */
class UserVip1Medal extends BaseMedal {
	
	protected $_is_UserVip1;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isUserVip1($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isUserVip1($uid) ) {
		...
		$result['next_level_time']	= $this->__isUserVip1($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isUserVip1($uid) {
		if (!isset($this->_is_UserVip1)) {
			$this->_is_UserVip1 = false;
			 $map['uid']=$uid;	
			 $map['issucess']=1;	
			 $map['tosucess']=1;	
  		 	 $sum=M('user_pay')->where($map)->sum('amount');
				if($sum > 10){
					$this->_is_UserVip1 = true;
				}
		}
		
		return $this->_is_UserVip1;
	}
