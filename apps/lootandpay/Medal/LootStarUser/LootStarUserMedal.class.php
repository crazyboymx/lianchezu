 ...
 * 程序_小时代 提供
 */
class LootStarUserMedal extends BaseMedal {
	
	protected $_is_LootStarUser;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isLootStarUser($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isLootStarUser($uid) ) {
		...
		$result['next_level_time']	= $this->__isLootStarUser($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isLootStarUser($uid) {
		if (!isset($this->_is_LootStarUser)) {
			$this->_is_LootStarUser = false;
			 $map['uid']=$uid;	
  		 	 $counts=M('loot_count')->where($map)->getField('counts');
				if($counts > 50){
					$this->_is_LootStarUser = true;
				}
		}
		
		return $this->_is_LootStarUser;
	}
