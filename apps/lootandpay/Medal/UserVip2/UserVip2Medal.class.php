 ...
 * 程序_小时代 提供
 */
class UserVip2Medal extends BaseMedal {
	
	protected $_is_UserVip2;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isUserVip2($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isUserVip2($uid) ) {
		...
		$result['next_level_time']	= $this->__isUserVip2($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isUserVip2($uid) {
		if (!isset($this->_is_UserVip2)) {
			$this->_is_UserVip2 = false;
			 $map['uid']=$uid;	
			 $map['issucess']=1;	
			 $map['tosucess']=1;	
  		 	 $sum=M('user_pay')->where($map)->sum('amount');
				if($sum > 50){
					$this->_is_UserVip2 = true;
				}
		}
		
		return $this->_is_UserVip2;
	}
