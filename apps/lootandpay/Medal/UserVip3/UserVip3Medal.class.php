 ...
 * 程序_小时代 提供
 */
class UserVip3Medal extends BaseMedal {
	
	protected $_is_UserVip3;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isUserVip3($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isUserVip3($uid) ) {
		...
		$result['next_level_time']	= $this->__isUserVip3($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isUserVip3($uid) {
		if (!isset($this->_is_UserVip3)) {
			$this->_is_UserVip3 = false;
			 $map['uid']=$uid;	
			 $map['issucess']=1;	
			 $map['tosucess']=1;	
  		 	 $sum=M('user_pay')->where($map)->sum('amount');
				if($sum > 100){
					$this->_is_UserVip3 = true;
				}
		}
		
		return $this->_is_UserVip3;
	}
