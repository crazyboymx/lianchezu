 ...
 * 程序_小时代 提供
 */
class LootNiuUserMedal extends BaseMedal {
	
	protected $_is_LootNiuUser;
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( $user_medal_data['data']['received_time'] <= 0 && $this->__isLootNiuUser($uid) ) {
			...
			}else if ( $user_medal_data['data']['received_time'] > 0 && !$this->__isLootNiuUser($uid) ) {
		...
		$result['next_level_time']	= $this->__isLootNiuUser($uid) ? '0' : '抢占星位';
		return $result;
	}
	...
	private function __isLootNiuUser($uid) {
		if (!isset($this->_is_LootNiuUser)) {
			$this->_is_LootNiuUser = false;
			 $map['uid']=$uid;	
  		 	 $counts=M('loot_count')->where($map)->getField('counts');
				if($counts > 20){
					$this->_is_LootNiuUser = true;
				}
		}
		
		return $this->_is_LootNiuUser;
	}
